...
for file in os.listdir(directory):
    ...
    if filename.endswith(".txt"): 
        ...
        # some dfs have 12 columns and some have 13
        # the last column is not actually any useful data (not Rank / affinity etc) so we can simply drop it 

        if df.shape[1] == 16:
            df = df.iloc[:, :-1]

        df.columns = ('Pos', 'MHC', 'Peptide', 'Of', 'Core', 'Core_Rel', 'Inverted', 'Identity', 'Score_EL', '%Rank_EL', 'Exp_Bind', 'Score_BA', '%Rank_BA', 'Affinity(nM)', 'BindLevel')
