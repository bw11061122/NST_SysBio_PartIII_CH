...
plt.savefig(f'/Users/barbarawalkowiak/Desktop/msc_thesis/project_report/figures_v2/figure3/{timestr}/{timestr}_method_UKBB_{param}_for_all_compare_to_ref_set_2k_jitter_mhc2.pdf', bbox_inches='tight')

# %%

# in case we need a plot with number of carriers indicated 
df_carriers_counts = ch_hla_merge_sub_all['gene_var'].value_counts().reset_index()
df_carriers_counts.columns.values[1] = 'nr_carriers'

df_compare_to_ref2 = pd.merge(df_compare_to_ref, df_carriers_counts, on = 'gene_var')
df_compare_to_ref2['nr_carriers'] = df_compare_to_ref2['nr_carriers'].astype(str)
df_compare_to_ref2['CH variant3'] = df_compare_to_ref2['gene_var2'] + '\n(n=' + df_compare_to_ref2['nr_carriers'] + ')'

# %%
# ordering based on scores in carriers only 
df_compare_to_ref2_carr = df_compare_to_ref2[df_compare_to_ref2['CH_variant_carrier']=='Carrier']
df_compare_to_ref2_carr['median_score'] = df_compare_to_ref2_carr.groupby('CH variant3')['score'].transform('median')
df_compare_to_ref2_sort = df_compare_to_ref2_carr.sort_values(by='median_score', ascending = False)
order = df_compare_to_ref2_sort['CH variant3'].unique()

# %%
plt.figure(figsize=(16,4)) # set figure size

sns.stripplot(y=f'score', x='CH variant3', hue = 'CH_variant_carrier', data=df_compare_to_ref2, dodge = True, jitter = True, palette = colors, size = 4, alpha = 0.3, legend = True, order = order)
